...
impressions_millions <- fb_ads |>
  ...
  ## Histogram heavily tilted towards lower amount of impressions, most candidates didn't have more than 10 million ads impressions.
```
...
house_top15 <- fb_UShouse |>
  ...
  select(cand_name_last, impressions, party) |>
  slice_head(n = 15)

house_top15

fb_top_plot <- ggplot(data = house_top15, mapping = aes(y = reorder(cand_name_last, impressions,), x = impressions / 1e6, fill = party)) +
