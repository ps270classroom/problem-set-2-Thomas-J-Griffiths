...
## There are 7014 candidates in the data set

incumbency_table <- fb_ads |>
  group_by(incumbency) |>
    summarize(candiate_count = n())
...
party_incumbent_promote <- fb_ads |>
 filter(office == "US Senate" | office == "US House") |>
  mutate(promote_prop = mean(ad_tone_promote)) |>
  group_by(incumbency, party) |>
  summarize(promote_prop = mean(ad_tone_promote, na.rm = TRUE))
party_incumbent_promote

knitr::kable(digits = 3, party_incumbent_promote, col.names = c("Incumbency", "Party", "Promote Ad Proportion"))
  
## Type of candidate with highest average ad_tone_promote are Incumbent Democrats at 85.4%
```
...
impressions_millions <- fb_ads |>
  ...
  fb_UShouse <- impressions_millions |>
    filter(office == "US House")
  
  plot_q3 <- ggplot(data = fb_UShouse, mapping = aes(x = impressions_millions)) +
    geom_histogram(color = "white")
  print(plot_q3)
  
Histogram heavily tilted towards lower amount of impressions, most candidates didn't have more than 10 million ad impressions.
```
...
```{r}
house_top15 <- fb_UShouse |>
  arrange(desc(impressions)) |>
  select(cand_name_last, impressions, party, impressions_millions) |>
  slice_head(n = 15)

house_top15

house_top15_new <- house_top15 |>
  mutate(cand_name_last = fct_reorder(cand_name_last, impressions_millions))

fb_top_plot <- ggplot(data = house_top15_new, mapping = aes(y = cand_name_last, x = impressions_millions, fill = party)) +
